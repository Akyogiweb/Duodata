...
const MappingButton = ({ children, active, side, index, onActivate }) => (
  <button
    ...
    onClick={onActivate}
    aria-pressed={active}
    className={`min-h-12 w-full border px-3 py-2 text-left text-sm font-semibold transition-[background-color,border-color,color,transform] duration-300 md:px-5 ${
      side === 'right' ? 'md:text-right' : ''
    } ${active ? 'border-cyan-500 bg-slate-950 text-white md:translate-x-0' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-400'}`}
    ...
    <section id="why-duo" className="border-y border-black/5 bg-white py-24 md:py-32" data-testid="two-sides-section">
      <div className="mx-auto max-w-[1200px] px-6">
        <Reveal className="grid gap-8 border-b border-slate-200 pb-12 md:grid-cols-[180px_minmax(0,1fr)] md:gap-16">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-500" data-testid="two-sides-eyebrow">04 / The bridge</p>
        ...
        <div className="mt-14" data-testid="two-sides-container">
          <div className="grid grid-cols-[minmax(0,1fr)_48px_minmax(0,1fr)] items-end gap-2 border-b border-slate-200 pb-4 md:grid-cols-[minmax(0,1fr)_128px_minmax(0,1fr)] md:gap-6">
            <div>
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Business</span>
            ...
            <div className="text-right">
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Technical</span>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex h-12 items-center justify-center" aria-hidden="true">
                    ...
                    <motion.div
                      className="absolute left-0 h-px bg-cyan-500"
                      animate={{ width: isActive ? '100%' : '0%' }}
                      transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
                    />
                    <span className={`relative grid h-8 w-8 place-items-center border bg-white transition-[border-color,color] duration-300 ${isActive ? 'border-cyan-500 text-cyan-700' : 'border-slate-200 text-slate-400'}`}>
                      <ArrowRight size={14} />
                    </span>
                  </div>
          ...
          <div className="mt-6 grid gap-3 border-l-2 border-cyan-500 bg-slate-50 px-5 py-5 md:grid-cols-[180px_minmax(0,1fr)] md:gap-10" data-testid="two-sides-active-detail">
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Active connection</span>
