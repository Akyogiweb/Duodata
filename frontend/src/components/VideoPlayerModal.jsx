...
const VideoPlayerModal = ({ video, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/90 p-4 md:p-8" role="dialog" aria-modal="true" aria-labelledby="video-modal-title" data-testid="video-player-modal">
      ...
      <div className="relative z-10 w-full max-w-5xl border border-white/15 bg-[#050505]">
        <div className="flex items-center justify-between gap-4 border-b border-white/10 px-4 py-3 md:px-5">
          <div className="min-w-0">
            <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-cyan-300">{video.category}</p>
          ...
          <button type="button" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center border border-white/15 text-white transition-colors duration-200 hover:border-cyan-400 hover:text-cyan-300" aria-label="Close video player" data-testid="video-modal-close-button">
            <X size={18} />
          </button>
        </div>
