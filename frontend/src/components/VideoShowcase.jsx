...
const VideoShowcase = ({ page = false }) => {
  ...
  return (
    <section id="videos" className={`border-y border-white/10 bg-[#07080c] text-white ${page ? 'pt-36 pb-24 md:pt-44 md:pb-28' : 'py-24 md:py-28'}`} data-testid="video-showcase-section">
      <div className="mx-auto max-w-[1440px] px-5 md:px-8">
        <header className="mx-auto mb-12 grid max-w-[1200px] gap-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          {!page && (
            <Link to="/videos" className="inline-flex items-center gap-2 border-b border-[#7FD1E8] pb-1 text-sm font-semibold text-white transition-colors duration-200 hover:text-[#7FD1E8]" data-testid="video-showcase-view-all-link">
          ...
          {videos.map((video, index) => {
            ...
            return (
              <motion.button
                ...
                className={`group relative min-h-0 min-w-0 overflow-hidden rounded-xl border text-left focus-visible:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#7FD1E8] ${isActive ? 'border-[#1E5FEE]/80' : 'border-white/10 bg-[#0c1018]'}`}
                  ...
                  {isActive ? (
                    <motion.div key="active" className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduced ? 0 : 0.35 }}>
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-5 md:p-7">
                        <div className="mb-4 grid h-11 w-11 place-items-center rounded-full border border-white bg-white text-black transition-[background-color,color,transform] duration-300 group-hover:scale-105 group-hover:bg-[#7FD1E8]" data-testid={`video-play-icon-${index + 1}`}><Play size={17} fill="currentColor" /></div>
                        <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#7FD1E8]">0{index + 1} / {video.category}</p>
        ...
        <div className="mx-auto mt-5 flex max-w-[1400px] items-center gap-2" aria-hidden="true">
          {videos.map((video, index) => <span key={video.id} className={`h-px flex-1 transition-colors duration-300 ${active === index ? 'bg-[#7FD1E8]' : 'bg-white/15'}`} />)}
        </div>
      </div>
